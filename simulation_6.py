...
class Server:
    ...
    def start_service(self, event, event_list, queue_time = 0):
        #self.update_busy_time(event.event_time)
        self.state = 1
        service_time = max(1e-3, self.service_time_generator())
        self.services.append(service_time)

        completion_time = event.event_time + service_time
        self.start_times.append(event.event_time)

        self.queue_waits.append(queue_time)

        if self.debug:
            print(f"t={event.event_time:.2f} | START | {self.name} serving {event.op_index} "
                  f"(completion at {completion_time:.2f})")

        event.event_time = completion_time
        heapq.heappush(event_list, (completion_time, "C", self, event))

    def handle_departure(self, event, event_list, completion_time, pool=None):
        #self.update_busy_time(completion_time)
        self.completed.append(completion_time)
        self.response_times.append(self.completed[-1] - self.arrivals[-1])
        ...
        if pool:
            pool.start_next(event_list, completion_time)
    ...
    def utilization(self, current_time):
        total_busy_time = sum(st for st in self.services)
        return total_busy_time / max(1e-9, current_time)
    ...
    def num_in_system_per_event(self):
        """Numero di job in sistema dopo ogni evento di arrivo o completamento"""
        in_system = 0
        result = []
        events = sorted([(t, 'A') for t in self.arrivals] +
                        [(t, 'D') for t in self.completed])
        for _, typ in events:
            if typ == 'A':
                in_system += 1
            else:
                in_system -= 1
            result.append(in_system)
        return result

    def avg_queue_population(self):
        nums = self.num_in_system_per_event()
        queue_lengths = [max(0, n - 1) for n in nums]
        return sum(queue_lengths) / len(queue_lengths) if queue_lengths else 0.0

    def avg_system_population(self):
        nums = self.num_in_system_per_event()
...
class ServerPool:
    ...
    def start_next(self, event_list, completion_time):
        while self.queue:
            free_server = next((s for s in self.servers if s.state == 0), None)
            if not free_server:
                break
            next_event = self.queue.popleft()
            queue_time = completion_time - next_event.event_time
            next_event.event_time = completion_time
            if self.debug:
                print(f"{free_server.name} serving next event from queue")
            free_server.arrivals.append(next_event.event_time)
            free_server.num_in_system += 1
            free_server.start_service(next_event, event_list, queue_time)
    ...
    def avg_wait_time(self):
        total_wait = sum((s.avg_wait_time() * len(s.completed)) for s in self.servers)
        total_jobs = sum(len(s.completed) for s in self.servers)
        return total_wait / max(1, total_jobs)

    def avg_response_time(self):
        total_resp = sum((s.avg_response_time() * len(s.completed)) for s in self.servers)
        total_jobs = sum(len(s.completed) for s in self.servers)
        return total_resp / max(1, total_jobs)

    def avg_queue_population(self):
        pops = [s.avg_queue_population() for s in self.servers]
        return sum(pops) / len(self.servers)

    def avg_system_population(self):
        pops = [s.avg_system_population() for s in self.servers]
        return sum(pops) / len(self.servers)
...
class ClassicCheckInServer(Server):
    def __init__(self, name):
        super().__init__(name, lambda: lognormal(5.1895, 0.08319))

class BagDropCheckInServer(Server):
    def __init__(self, name):
        super().__init__(name, lambda: lognormal(4.064, 0.246))
...
class AirportSimulation:
    ...
    def generate_new_arrival(self, op_index=None):
        ...
        event_time = self.last_arrival_time.get(op_index, 0.0) + interarrival
    ...
    def process_next_event(self):
        ...
        if event_type in ["A", "E"]:
            ...
            server.handle_departure(event, self.event_list, pool=pool, completion_time=time)
    ...
    def collect_metrics(self):
        total_util = sum(s.utilization(self.times.next) for s in self.servers)
        avg_util = total_util / len(self.servers)

        snapshot = {
            "time": self.times.next,
            "global": {
                "total_in_system": sum(pool.total_in_system() for pool in self.server_pools.values()),
                "passengers_completed": self.exiting_jobs,
                "avg_utilization": avg_util,
                "avg_queue_length": sum(len(pool.queue) for pool in self.server_pools.values()) / len(self.server_pools)
            },
            "servers": [
                {
                    "name": s.name,
                    ...
                    "busy": s.state,
                    "utilization": s.utilization(self.times.next)
    ...
    def collect_metrics_infinite(self):
        ...
        Deve essere chiamato periodicamente (ad ogni evento o step).
        """
        ...
        for pool_name, pool in self.server_pools.items():
            ...
            for s in pool.servers:
                ...
                test = s.utilization(self.times.next)
                print("bho")
            ...
            if pool_name == "turnstiles" or pool_name == "fast_track_turnstile":
                pool_name = "turnstile_area"
            lambda_rescaled = lambdas[pool_name] * multiplier
    ...
    def run(self):
        ...
        if self.type_simulation == "finite":
            ...
            while self.processed_batch < self.batch_num:
                ...
                if self.completed_jobs == self.next_sampling and self.completed_jobs != 0:
