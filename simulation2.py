...
class ClassicCheckInServer(Server):
    def __init__(self, name):
        super().__init__(name,
            lambda: truncate_lognormal(5.1895, 0.2, 30, 300))  # sigma aumentato da 0.08319 a 0.2

class BagDropCheckInServer(Server):
    def __init__(self, name):
        super().__init__(name,
            lambda: truncate_lognormal(4.064, 0.5, 20, 180))  # sigma aumentato da 0.246 a 0.5
...
class AirportSimulation:
    ...
    def update_current_slot(self, current_time):
        for start, end, name in self.time_slots:
            if start <= current_time < end:
                ...
                self.current_lambda = lambda_scaler.return_new_lambdas(self.current_slot)
                break
    ...
    def init_servers_and_pools(self):
        if self.type_simulation == "infinite":
            ...
            self.server_pools = {
                ...
                "premium_economy":          ServerPool([ClassicCheckInServer(f"premium_economy_{index}") for index in range(2)]),
                "economy":                  ServerPool([ClassicCheckInServer(f"economy_{index}") for index in range(3)]),
                "flexi_plus":               ServerPool([ClassicCheckInServer("flexi_plus_0")]),
                "self_bd":                  ServerPool([BagDropCheckInServer(f"self_bd_{index}") for index in range(3)]),
    ...
    # -------------------- Interarrival --------------------
    def generate_interarrival_time(self, lambda_to_use):
      return exponential(1 / lambda_to_use)
    ...
    def generate_new_arrival(self):
        select_stream(99)
        lambda_a = self.current_lambda * 0.6

        r1 = random()
        trad_or_lowcost = "trad" if r1 < 0.2 else "lowcost"
        if trad_or_lowcost == "trad":
            r2 = random()
            op_index = (
                "business" if r2 < 0.10 else
                "premium_economy" if r2 < 0.30 else  # 0.10 + 0.20
                "economy" if r2 < 0.75 else  # 0.30 + 0.45
                "turnstile_area_external"
            )
        else :
            r3 = random()
            turnstile_or_no = "turnstile_area" if r3 < 0.7 else "no_turn"
            if turnstile_or_no == "turnstile_area":
                op_index = "turnstile_area_external"
            else :
                r4 = random()
                op_index = (
                    "flexi_plus" if r4 < 0.05 else
                    "self_bd" if r4 < 0.15 else  # 0.05 + 0.10
                    "bd"
                )
        interarrival = self.generate_interarrival_time(lambda_a)
        if self.last_arrival_time["endogenous"] + interarrival <= self.end_time:
            event_time = self.last_arrival_time["endogenous"] + interarrival
            self.last_arrival_time["endogenous"] = event_time
    ...
    def generate_exogenous_arrival(self):
        select_stream(30)
        lambda_e = self.current_lambda * 0.4
        interarrival = self.generate_interarrival_time(lambda_e)
        #event_time = max(self.last_arrival_time.get("exogenous", 0.0) + interarrival, self.times.next + interarrival)
        if self.last_arrival_time["exogenous"] + interarrival <= self.end_time:
            transit_time = self.transit_time_exogenous() / 10
            arrival_time = self.last_arrival_time["exogenous"] + interarrival +  transit_time
            self.last_arrival_time["exogenous"] = arrival_time
            event = Event(arrival_time, "E", "turnstile_area_external")
            event.exogenous = True
            heapq.heappush(self.event_list, (arrival_time, "E", event))
            print(f"  [SCHEDULE] Exogenous B+C passenger at t={arrival_time:.2f}")
        else :
            self.counter += 1
